...
/**
 * @description True when the email is a synthetic placeholder generated by
 * the Lark OAuth provider for tenants whose directory does not expose a
 * real email. Backend rule of record: `apps/api/plane/utils/lark_notify.py`
 * (`email.endswith("@lark.local")`). Keep these two in sync.
 */
export const isSyntheticAuthEmail = (email?: string | null): boolean => {
  if (!email) return false;
  return email.toLowerCase().endsWith("@lark.local");
};

/**
 * @description Returns the email if it is safe to render in user-facing UI,
 * or undefined when it is a synthetic auth placeholder that would only leak
 * an internal identifier (e.g. `n_<union_id>@lark.local`). Use this in any
 * profile widget that shows the email under a display name.
 */
export const getDisplayableUserEmail = (email?: string | null): string | undefined => {
  if (!email || isSyntheticAuthEmail(email)) return undefined;
  return email;
};
