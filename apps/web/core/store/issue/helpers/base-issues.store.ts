...
export abstract class BaseIssuesStore implements IBaseIssuesStore {
    ...
    try {
      ...
      runInAction(() => {
        for (const updateItem of updates) {
          ...
          if (updateItem.start_date) dates.start_date = updateItem.start_date;
          if (updateItem.target_date) dates.target_date = updateItem.target_date;

          const currIssue = getIssueById(updateItem.id);
          ...
          this.issueUpdate(workspaceSlug, projectId, updateItem.id, dates, false);
        }
      });
      ...
      runInAction(() => {
        for (const updateItem of issueDatesBeforeChange) {
          const dates: Partial<TIssue> = {};
          if (updateItem.start_date) dates.start_date = updateItem.start_date;
          if (updateItem.target_date) dates.target_date = updateItem.target_date;
  ...
  updateGroupedIssueIds(
    ...
    for (const grpId in groupedIssues) {
      const issueGroup = groupedIssues[grpId];
      const issueGroupCount = groupedIssueCount[grpId];

      // update the group's issue count
      set(this.groupedIssueCount, [grpId], issueGroupCount);

      // This updates the group issue list in the store, if the issueGroup is a string
      const storeUpdated = this.updateIssueGroup(issueGroup, [grpId]);
      ...
      for (const subGrpId in issueGroup) {
        const issueSubGroup = (issueGroup as TGroupedIssues)[subGrpId];
        const issueSubGroupCount = groupedIssueCount[getGroupKey(grpId, subGrpId)];

        // update the sub group's issue count
        set(this.groupedIssueCount, [getGroupKey(grpId, subGrpId)], issueSubGroupCount);
        // This updates the subgroup issue list in the store
        this.updateIssueGroup(issueSubGroup, [grpId, subGrpId]);
      }
    }
  }
  ...
  issuesSortWithOrderBy = (issueIds: string[], key: TIssueOrderByOptions | undefined): string[] => {
    // Custom-field sort is computed server-side (apply_custom_field_order):
    // the value isn't on the client TIssue, so any client re-sort here would
    // just scramble the server order back to -created_at. Trust the server
    // order and return the ids unchanged.
    if (key && /^-?custom_field__/.test(key)) return issueIds;
